...
{	
	...
	ret2 = mutex_create();
	check_return(errno,ENOMEM,"4. Mutex Create returns ENOMEM");
	ret2 = mutex_unlock(165);
	check_return(errno,EINVAL,"5. Mutex Unlock return EINVAL");
	ret2 = mutex_unlock(23);
	check_return(errno,EPERM,"6. Mutex Unlock return EPERM");
	ret2 = mutex_lock(124);
	check_return(errno,EINVAL,"7. Mutex Lock return EINVAL");
	ret2 = mutex_lock(4);
	check_return(ret2,0,"8. Mutex Lock return success");
	ret2 = mutex_lock(4);
	check_return(errno,EDEADLOCK,"9. Mutex Lock return EDEADLOCK");
	ret2 = mutex_unlock(4);
	check_return(ret2,0,"10. Mutex Unlock return success");
	ret2 = event_wait(10000);
	check_return(errno,EINVAL,"11. Event Wait return EINVAL");
	puts("TEST END!\n");
	while(1) {
		spin++;
	}
...
int main(int argc, char** argv)
{
	...
	// this should return ESCHED
	ret = task_create(tasks, 2);
	check_return(errno,ESCHED,"3. Test create return ESCHED");
